...
def register_connection_routes(mcpserver):
    ...
    async def create_database_type(request: Request):
        ...
        row = await repository_manager.add_type(data["name"])
    ...
    async def add_connection(request: Request):
        ...
        row = await repository_manager.add_connection(data)
        return JSONResponse(content=dict(row))
    ...
    async def update_connection(request: Request):
        ...
        if "password" in data:
            ...
            row = await repository_manager.update_connection(connection_id, data)
        else:
            row = await repository_manager.update_connection_no_password(connection_id, data)
    ...
    async def delete_connection(request: Request):
        connection_id = int(request.path_params["connection_id"])
        await repository_manager.delete_connection(connection_id)
        return JSONResponse(content={"status": "deleted"})
    ...
    async def deactivate_all_connections(request):
        ...
        try:
            result = await repository_manager.deactivate_all_connections()
    ...
    async def activate_connection(request):
        ...
        try:
            ...
            result = await repository_manager.activate_connection(connection_id)
    ...
    async def deactivate_connection(request):
        ...
        try:
            connection_id = request.path_params["connection_id"]
            result = await repository_manager.deactivate_connection(connection_id)
            status = 200 if result.get("status", "ok") == "ok" or "id" in result else 404
            return JSONResponse(content=result, status_code=status)
        except Exception as e:
            return JSONResponse(
